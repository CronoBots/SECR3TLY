  ...
  function plan(el) {
    ...
    el.innerHTML = '<div class="plans">' + S.PLANS.map(function (p) {
      ...
      return '<article class="plan' + (on ? ' on' : '') + (p.id === 'premium' ? ' feat' : '') + '"><div class="row"><h3 class="plan-n">' + esc(p.name) + '</h3>' + (on ? '<span class="pill pill-ok">Plan actuel</span>' : p.id === 'pro' ? '<span class="pill pill-mute">' + esc(p.tag) + '</span>' : '') + '</div>' +
        ...
        (on ? '<button type="button" class="btn btn-ghost btn-sm" disabled>Plan actuel</button>' : p.id === 'business' ? '<button type="button" class="btn btn-ghost btn-sm" data-contact>Nous contacter</button>' : (function () { var ids = S.PLANS.map(function (x) { return x.id; }), up = ids.indexOf(p.id) > ids.indexOf(cur); return '<button type="button" class="btn btn-sm ' + (!up ? 'btn-ghost' : p.id === 'premium' ? 'btn-sig' : 'btn-primary') + '" data-plan="' + p.id + '">' + (up ? 'Passer à ' : 'Revenir à ') + esc(p.name) + '</button>'; })()) + '</article>';
    }).join('') + '</div>' +
